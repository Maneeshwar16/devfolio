...
export default function Container(props: ContainerProps) {
  ...
  // preloader effect
  useEffect(() => {
    // Total time: 8 greetings * 300ms = 2400ms (2.4 seconds) + buffer = 3000ms (3 seconds)
    setTimeout(() => {
      setIsLoading(false);
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:url"
          content={`https://maneeshwar-portfolio.vercel.app${router.asPath}`}
        />
        <link
          rel="canonical"
          href={`https://maneeshwar-portfolio.vercel.app${router.asPath}`}
        />
        <meta property="og:type" content={meta.type} />
        <meta property="og:site_name" content="Maneeshwar" />
        <meta property="og:description" content={meta.description} />
        <meta property="og:title" content={meta.title} />
        <meta property="og:image" content={meta.image} />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:site" content="Maneeshwar" />
      ...
      >
        ...
        <Link href="/">
          <span className="text-lg font-semibold">maneeshwar</span>
        </Link>
              ...
              <div className="flex h-full flex-col items-start justify-between overflow-y-auto">
                ...
                <div className="flex min-h-fit w-full flex-col space-y-8 px-[22px] py-10">
                  <span className="text-sm text-muted-foreground">
                    © {new Date().getFullYear()} maneeshwar. All rights reserved.
                  </span>
                </div>
              </div>
            </motion.div>
